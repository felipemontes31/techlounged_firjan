...
<!DOCTYPE html>
<html lang="pt-BR">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Eixos tecnológicos | TechLounged</title><link rel="stylesheet" href="<?= BASE_URL ?>/assets/css/techlounged.css"></head>
<body>
<?php include(__DIR__ . '/../../includes/topo.php'); ?>
<?php include(__DIR__ . '/../../includes/admin_sidebar.php'); ?>
<main class="tl-main">
  <div class="tl-admin-top"><div><h1>Eixos tecnológicos</h1><p class="tl-meta">Cadastro administrativo restrito a administrador e bibliotecário.</p></div></div>
  <section class="tl-card tl-card-pad" style="margin-bottom:18px;">
    <form id="formCrud" class="tl-form-grid">
      <input type="hidden" id="id" name="id">
      <div class="tl-field"><label>Nome do eixo</label><input type="text" id="nome_eixo" name="nome_eixo" placeholder="Ex: Tecnologia" required></div>
      <div class="tl-field tl-full"><label>Observação</label><textarea id="observacao" name="observacao" placeholder=""></textarea></div>
  ...
  <section class="tl-table-wrap"><table class="tl-table"><thead><tr><th>ID</th><th>Eixo</th><th>Observação</th><th>Ações</th></tr></thead><tbody id="tabelaCorpo"></tbody></table></section>
</main></div>
<script src="<?= tl_url('assets/js/techlounged.js') ?>"></script>
<script>
const urlController = '<?= BASE_URL ?>/services/eixo_controle.php';
const colunas = [["id", "ID"], ["nome_eixo", "Eixo"], ["observacao", "Observação"]];
